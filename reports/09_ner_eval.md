...
- Generated at: 2026-04-21 10:03:09 UTC
- Model: `microsoft/deberta-v3-base (fine-tuned NER)`
- Test samples: 10
...
- masking_recall: 0.97 (97.06%)
- over_masking_rate: 0.03 (2.94%)
- text_preservation: 0.93 (93.31%)
...
```text
Hello Mrs[PREFIX] Walker, can you check why my recent transfer of $[AMOUNT] hasn't been processed? My account number is 567[ACCOUNT_NUMBER] and my email is me[EMAIL].
```
...
```text
Dear Ms[PREFIX] Garcia, I need to report a lost debit card. My phone number is ([PHONE_NUMBER] and my account number is 678[ACCOUNT_NUMBER].
```
...
```text
I'd like to know if there are any charges incurred when transferring the payment to my bank account with SQ[BIC]. If yes, could you please tell me in details?
```
...
```text
Hi Mrs[PREFIX] Brown, I need to reset my online banking password. My username is michael[USERNAME] and my phone number is ([PHONE_NUMBER].
```
...
```text
"Hello, Jordan[FIRSTNAME] Volk[LASTNAME], we have detected irregular activities on our Money[ACCOUNTNAME] account number 86[ACCOUNT_NUMBER] with a transaction amount of 668[AMOUNT][CURRENCY]. Can you please look into this matter and ensure that our procedures were followed correctly?"
```
...
```text
For your financial protection, we would like to confirm the credit card 937[CREDITCARDNUMBER] with CVV 174 is authorized for payments. A test transaction of [CURRENCYSYMBOL][AMOUNT] will be processed.
```
...
```text
Hi Mr[PREFIX] Scott, I have a question about my loan repayment. My name is Ethan[NAME] and my account number is 90[ACCOUNT_NUMBER].
```
...
```text
Hi Mr[PREFIX] Miller, I need assistance with my credit card rewards program. My username is joseph[USERNAME] and my SSN is 444[SSN][PHONE_NUMBER].
```
...
```text
Hello Mr[PREFIX] Brown, my debit card was declined. My account number is 789[ACCOUNT_NUMBER] and my phone number is ([PHONE_NUMBER].
```
...
```text
Dear Ms[PREFIX] Nelson, can you please update my contact number to ([PHONE_NUMBER]? My account number is 789[ACCOUNT_NUMBER] and my email is is[EMAIL].
```
